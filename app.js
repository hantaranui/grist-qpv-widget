(function () {
  ...
  const COLUMN_SCHEMA = [
    ...
    { name: "statut", title: "Statut verification QPV", type: "Text", optional: true }
  ];
  ...
  grist.onRecord(async (record, mappings) => {
    ...
    if (!state.current || !hasAddressMapping()) {
      showMessage("Selectionnez une ligne et associez une colonne d'adresse.");
      clearResult();
      return;
    }
  ...
  async function runCurrentCheck(writeBack) {
    if (!state.current) {
      showMessage("Aucune ligne selectionnee.");
      return;
    }

    try {
      setBusy(true, "Verification de l'adresse...");
      ...
      if (writeBack) {
        ...
        showMessage("Resultat ecrit dans Grist.");
      } else {
  ...
  async function runBatchCheck() {
    if (!state.records.length) {
      showMessage("Aucune ligne a traiter.");
      return;
    }

    try {
      setBusy(true, "Verification de toutes les lignes...");
      await loadQpvData();
      const actions = [];
      let done = 0;
      ...
      for (const record of state.records) {
        ...
        if (done % 10 === 0) {
          showMessage(`${done}/${state.records.length} lignes verifiees...`);
        }
      }
      ...
      showMessage(`${done} lignes verifiees.`);
    } catch (error) {
      renderError(error);
    } finally {
      setBusy(false);
  ...
  async function checkRecord(record) {
    ...
    if (!hasUsableStreetAddress(record)) {
      return resultFor(record, null, null, "Adresse de rue absente : la colonne mappee ne contient pas une adresse exploitable.");
    }

    await loadQpvData();
    const geocoded = await geocode(address);
    if (!geocoded) {
      return resultFor(record, null, null, "Adresse non trouvee par la BAN.");
    }

    const point = geocoded.geometry.coordinates;
    const feature = findContainingQpv(point, state.qpvFeatures);
    const status = feature ? "Adresse situee dans un QPV." : "Adresse hors QPV.";
  ...
  async function loadQpvData(options = {}) {
    ...
    if (cached && !options.force) {
      ...
      showMessage(`Contours QPV charges depuis le cache (${cached.features.length} zones).`);
      return;
    }
    ...
    if (!resource) {
      throw new Error("Aucune ressource GeoJSON QPV trouvee sur data.gouv.fr.");
    }

    const geojson = await fetchGeojsonZip(resource.latest || resource.url);
    const features = Array.isArray(geojson.features) ? geojson.features : [];
    if (!features.length) {
      throw new Error("Le fichier QPV charge ne contient aucune zone.");
    }
    ...
    showMessage(`Contours QPV charges (${features.length} zones).`);
  }
  ...
  async function fetchGeojsonZip(url) {
    ...
    if (!response.ok) {
      throw new Error(`Telechargement QPV impossible (${response.status}).`);
    }
    ...
    if (!file) {
      throw new Error("Aucun fichier GeoJSON trouve dans l'archive QPV.");
    }
  ...
  async function geocode(address) {
    ...
    if (!response.ok) {
      throw new Error(`Geocodage BAN impossible (${response.status}).`);
    }
  ...
  async function writeResult(rowId, result) {
    ...
    if (!action) {
      showMessage("Aucune colonne de sortie n'est associee.");
      return;
    }
  ...
  function writeCache(features, metadata) {
    try {
      ...
      // Le widget reste utilisable meme si le cache navigateur est plein ou indisponible.
    }
  }
